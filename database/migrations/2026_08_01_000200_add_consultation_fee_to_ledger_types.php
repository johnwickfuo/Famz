...
{
    /**
     * Widen the ledger's type column for consultation fees.
     *
     * The column is an enum, which means the set of allowed values lives in the
     * schema as well as in PHP; adding a case to the one without the other
     * produces a write that fails at the database rather than in code.
     */
    ...
    public function down(): void
    {
        $this->setTypeEnum(array_values(array_filter(
            LedgerType::values(),
            fn (string $value): bool => $value !== LedgerType::ConsultationFee->value,
        )));
    }
